...
app.get("/wishlist/:userId", async (req, res) => {
  ...
  try {
    ...
    // Consultar los productos en la wishlist del usuario
    const query = `
      SELECT wp.id_producto, p.nombre, p.precio
      FROM WISHLIST_PRODUCTO wp
      JOIN PRODUCTO p ON wp.id_producto = p.id
      WHERE wp.id_usuario = ?`;
    ...
    res.status(500).json({ success: false, error: "Error al obtener la wishlist." });
  } finally {
    if (connection) {
      try {
        await connection.end();
